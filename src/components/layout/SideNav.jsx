...
import { Home, CheckSquare, RefreshCw, Settings, Plus } from 'lucide-react'
...
export default function SideNav({ onAddTransaction }) {
  ...
  return (
    <aside className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-56 border-r border-border dark:border-[#1A1A1A] bg-bg dark:bg-[#000000] z-sticky">
      <div className="px-5 h-16 flex items-center border-b border-border dark:border-[#1A1A1A]">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-md text-[14px] font-medium transition-colors duration-150 ${
                isActive
                  ? 'bg-bg-secondary dark:bg-[#111111] text-text dark:text-[#EDEDED]'
                  : 'text-text-muted dark:text-[#888888] hover:bg-bg-secondary dark:hover:bg-[#0A0A0A] hover:text-text dark:hover:text-[#EDEDED]'
              }`
            }
          >
