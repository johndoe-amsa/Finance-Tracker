...
// Module-level in-flight promise: prevents two concurrent invocations
// (e.g. a fast session change that re-fires the AuthGate effect, or the
// VerifyPage "refresh" button clicked during startup) from racing and
// inserting duplicate transactions. Later callers await the same run
// instead of being dropped, so their UI feedback reflects reality.
let inFlight = null
 ...
 */
export function generateMissingSubscriptionTransactions() {
  if (inFlight) return inFlight
  inFlight = runGeneration().finally(() => { inFlight = null })
  return inFlight
}
